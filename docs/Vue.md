...
Для зібрання проєкту та запуску сервера, у папці проєкту потрібно виконати команду:
```powershell
npm run serve
```
...
- Network: http://192.168.43.68:8080/
```
Проте, ця збірка не є оптимізованою, з метою економії часу під час розробки. Для створення оптимізованої збірки, потрібно виконати команду:
```powershell
npm run build
```
<br>
